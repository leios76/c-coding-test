...
void solve(struct context_t * ctx)
{
    ...
    for (int n = 0; n < ctx->n; n++) {
        if (ctx->queue.queue_pool[n].v == 0) {
            index = ctx->queue.queue_head_index;
            if (index == -1) {
                printf("%d\n", 0);
            } else {
                printf("%d\n", ctx->queue.queue_pool[index].v);
                remove_queue(&ctx->queue, ctx->queue.queue_head_index);
            }
        } else {
            index = ctx->queue.queue_head_index;
            if (index == -1) {
                insert_queue(&ctx->queue, -1, -1, n);
            } else {
                while (index != -1) {
                    if (ctx->queue.queue_pool[n].v < ctx->queue.queue_pool[index].v) {
                        insert_queue(&ctx->queue, -1, index, n);
                        break;
                    }
...
void init_data(struct context_t * ctx)
{
    ...
    scanf("%d", &ctx->n);
    for (int n = 0; n < ctx->n; n++) {
        scanf("%d", &ctx->queue.queue_pool[n].v);
    }
}
