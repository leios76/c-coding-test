...
void enqueue(struct heap_t * heap, int v)
{
    ...
    if (heap->is_min) {
        while (i > 1 && compare(v, heap->heap_pool[i / 2].v) < 0) {
...
void heapify(struct heap_t * heap, int i)
{
    ...
    if (heap->is_min) {
        if (left <= heap->heap_size && compare(heap->heap_pool[left].v, heap->heap_pool[i].v) < 0) {
            largest = left;
        }
        if (right <= heap->heap_size && compare(heap->heap_pool[right].v, heap->heap_pool[largest].v) < 0) {
            largest = right;
        }
    } else {
        if (left <= heap->heap_size && compare(heap->heap_pool[left].v, heap->heap_pool[i].v) > 0) {
