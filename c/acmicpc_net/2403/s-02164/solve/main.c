...
struct queue_t {
    struct queue_entity_t queue_pool[500001];
...
void solve(struct context_t * ctx)
{
    ...
    for (int n = 1; n <= ctx->n; n++) {
        ctx->queue.queue_pool[n].n = n;
        ctx->queue.queue_pool[n].prev_index = n - 1;
        ctx->queue.queue_pool[n].next_index = n + 1;
    }
    ctx->queue.queue_pool[1].prev_index = -1;
    ctx->queue.queue_pool[ctx->n].next_index = -1;
    ctx->queue.queue_head_index = 1;
    ctx->queue.queue_tail_index = ctx->n;

    for (int n = 1; n <= ctx->n; n++) {
        if (ctx->queue.queue_head_index == ctx->queue.queue_tail_index) {
            result = ctx->queue.queue_head_index;
            break;
        }
