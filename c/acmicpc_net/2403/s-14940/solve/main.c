...
void bfs(struct context_t * ctx)
{
    while (ctx->queue.queue_size > 0) {
        ...
        if (n > 0 && ctx->bfs_v[index - 1024] == 0) {
            ctx->queue.queue_pool[index - 1024].turn = ctx->queue.queue_pool[index].turn + 1;
            debug("[%d %d] add %d %d\n", n, m, n -1, m);
            insert_queue(&ctx->queue, ctx->queue.queue_tail_index, -1, index - 1024);
            ctx->bfs_v[index - 1024] = 1;
        }
        if (m > 0 && ctx->bfs_v[index - 1] == 0) {
            debug("[%d %d] add %d %d\n", n, m, n, m - 1);
        ...
        if (n < ctx->n - 1 && ctx->bfs_v[index + 1024] == 0) {
            debug("[%d %d] add %d %d\n", n, m, n + 1, m );
            ctx->queue.queue_pool[index + 1024].turn = ctx->queue.queue_pool[index].turn + 1;
            insert_queue(&ctx->queue, ctx->queue.queue_tail_index, -1, index + 1024);
            ctx->bfs_v[index + 1024] = 1;
        }
        if (m < ctx->m - 1 && ctx->bfs_v[index + 1] == 0) {
            debug("[%d %d] add %d %d\n", n, m, n , m + 1);
            ctx->queue.queue_pool[index + 1].turn = ctx->queue.queue_pool[index].turn + 1;
            insert_queue(&ctx->queue, ctx->queue.queue_tail_index, -1, index + 1);
            ctx->bfs_v[index + 1] = 1;
...
void solve(struct context_t * ctx)
{
    ...
    for (int n = 0; n < ctx->n; n++) {
        for (int m = 0; m < ctx->m; m++) {
            printf("%d ", ctx->queue.queue_pool[n * 1024 + m].turn);
        }
        printf("\n");
    }
}
...
void init_data(struct context_t * ctx)
{
    ...
    for (int n = 0; n < ctx->n; n++) {
        for (int m = 0; m < ctx->m; m++) {
            scanf("%d", &ctx->map[n * 1024 + m]);
            if (ctx->map[n * 1024 + m] == 2) {
                ctx->queue.queue_pool[n * 1024 + m].turn = 0;
                insert_queue(&ctx->queue, -1, -1, n * 1024 + m);
                ctx->bfs_v[n * 1024 + m] = 1;
            } else if (ctx->map[n * 1024 + m] == 0) {
                ctx->queue.queue_pool[n * 1024 + m].turn = 0;
                ctx->bfs_v[n * 1024 + m] = 1;
            }
        }
