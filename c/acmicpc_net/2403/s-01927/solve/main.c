...
void min_enqueue(struct heap_t * heap, int v)
{
...
void min_heapify(struct heap_t * heap, int i)
{
    int largest = i;
    int left = 2 * i;
    int right = 2 * i + 1;
...
int min_dequeue(struct heap_t * heap)
{
    ...
    for (int n = 0; n < ctx->n; n++) {
        if (ctx->v[n] == 0) {
            if (ctx->heap.heap_size == 0) {
                ...
                printf("%d\n", min_dequeue(&ctx->heap));
            }
        } else {
            min_enqueue(&ctx->heap, ctx->v[n]);
        }
    }
}
